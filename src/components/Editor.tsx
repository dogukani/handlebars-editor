...
export function HandlebarsEditor({
  ...
  // Calculate textarea styles for positioning
  const textareaStyles = useMemo(() => {
    if (!autocompleteState.isOpen || !textareaRef.current) {
      return null
    }
    const textarea = textareaRef.current
    const styles = getComputedStyle(textarea)
    const editorStyles = getComputedStyle(textarea.parentElement || textarea)
    return {
      lineHeight: parseFloat(styles.lineHeight) || 20,
      paddingTop: parseFloat(styles.paddingTop) || 0,
      paddingLeft: parseFloat(styles.paddingLeft) || 0,
      charWidth: (parseFloat(styles.fontSize) || 14) * 0.6,
      dropdownWidth:
        parseInt(editorStyles.getPropertyValue('--hbs-autocomplete-min-width'), 10) || 180,
      dropdownMaxHeight:
        parseInt(editorStyles.getPropertyValue('--hbs-autocomplete-max-height'), 10) || 200,
  ...
  const autocompletePosition = useMemo(() => {
    if (!autocompleteState.isOpen || !textareaStyles) {
      return { top: undefined, bottom: undefined, left: 0, maxHeight: 200, openUpward: false, visible: false }
    }

    const { triggerStart } = autocompleteState
    const {
      lineHeight,
      paddingTop,
      paddingLeft,
    ...
    const cursorTop = paddingTop + lineNumber * lineHeight - scrollOffset.top
    let left = paddingLeft + charInLine * charWidth - scrollOffset.left

    const maxLeft = clientWidth - dropdownWidth - 10
    if (left > maxLeft) {
      left = Math.max(10, maxLeft)
    }

    const spaceBelow = clientHeight - cursorTop - 10
    const spaceAbove = cursorTop - lineHeight - 10
    ...
    if (openUpward) {
      maxHeight = Math.min(dropdownMaxHeight, Math.max(80, spaceAbove))
      // Use bottom positioning so dropdown aligns to cursor regardless of actual height
      bottom = clientHeight - cursorTop + lineHeight
      top = undefined
    } else {
      maxHeight = Math.min(dropdownMaxHeight, Math.max(80, spaceBelow))
      top = cursorTop
      bottom = undefined
    }

    const visible =
      cursorTop >= 0 && cursorTop < clientHeight && left >= 0 && left < clientWidth

    return { top, bottom, left, maxHeight, openUpward, visible }
  }, [autocompleteState, value, scrollOffset, textareaStyles])
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        aria-expanded={autocomplete && autocompleteState.isOpen && autocompletePosition.visible}
      ...
      {autocomplete && autocompleteState.isOpen && autocompletePosition.visible && (
        <div
          ref={autocompleteRef}
          id={listboxId}
          role="listbox"
          aria-label="Autocomplete suggestions"
          className="hbs-autocomplete"
          style={{
            top: autocompletePosition.top,
            bottom: autocompletePosition.bottom,
            left: autocompletePosition.left,
            maxHeight: autocompletePosition.maxHeight,
          }}
        >
